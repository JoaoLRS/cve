# CDU-03: Processamento da imagem

1. A tela `Processamento imagem` contera os seguintes componentes:

    1.1 - Titulo: `Processamento da imagem: nome_do_arquivoImg`.

    1.2 - Filtros: Em baixo do titulo irar tem um campo com o nome do `filtro` e ao lado dele deve ter um `input` para cada configuração do filtro **(no input deve conter uma informação mostrado um exemplo)**.
